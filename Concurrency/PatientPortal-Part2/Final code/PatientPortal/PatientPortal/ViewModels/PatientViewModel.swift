...
final class PatientViewModel: ObservableObject, PatientViewModelProtocol {
    ...
    @Published var isPatientReportDownloaded: Bool = false
    ...
    func downloadReports(for patientId: Int) async throws {
        ...
        do {
            ...
            let reports = try await self.patientRepository.getPatientReports(for: patientId)
            try await self.patientRepository.savePatientReportsToDocumentDirectory(reports: reports, patientId: patientId)
            isPatientReportDownloaded = true
            
        }
        catch let error {
            isPatientReportDownloaded = false
